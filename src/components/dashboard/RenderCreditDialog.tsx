      ...
      >
        ...
        {hasEnough && (!plan || plan === "free") && (
          <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl"
            style={{ background: "hsl(45,100%,50%,0.08)", border: "1px solid hsl(45,100%,50%,0.25)" }}>
            <AlertTriangle className="w-4 h-4 text-yellow-400 flex-shrink-0 mt-0.5" />
            <p className="text-xs text-yellow-300/90">
              <span className="font-semibold">Free plan:</span> rendered clip will include a HookCut watermark.{" "}
              <Link to="/dashboard/upgrade" className="text-yellow-300 underline font-medium">{t("renderCredit.upgradeToRemove")}</Link>
            </p>
          </div>
        )}
